...
export default function FinancialOfficePage() {
  ...
  const loadFinancialData = async () => {
    ...
    try {
      ...
      setReferralLink(`https://deluxeisa.com/convite/${code}`)
      ...
      // Load network tree
      const treeData = await getCreatorNetworkTree(profile.username, 4) // 4 levels
  ...
  const renderNetworkTree = (nodes: any[], depth = 0) => {
    ...
    return nodes.map((node) => (
      <div key={node.id} className={`ml-${depth > 0 ? 4 : 0} mt-2`}>
        <div
          className={`flex items-center space-x-3 p-3 rounded-lg border-l-4 transition-all hover:shadow-md cursor-pointer ${
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold truncate">@{node.creatorUsername}</p>
            <div className="flex items-center gap-2 mt-1">
              <Badge variant="outline" className={`text-xs ${levelBadges[Math.min(depth, 3)]}`}>
                Nível {depth + 1}
              </Badge>
              <span className="text-xs text-muted-foreground">{node.subscriberCount || 0} assinantes</span>
            </div>
          </div>
          <div className="flex-shrink-0 text-right">
            <p className="text-sm font-bold text-green-600">R$ {((node.monthlyEarnings || 0) / 100).toFixed(2)}</p>
            <p className="text-xs text-muted-foreground">faturamento</p>
          </div>
        </div>
        {expandedNodes.has(node.id) && node.children?.length > 0 && (
          <div className="ml-4 mt-1 border-l-2 border-muted pl-2">{renderNetworkTree(node.children, depth + 1)}</div>
        )}
      </div>
    ))
  }
        ...
        <div className="py-6 space-y-6">
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <div className="space-y-1">
              <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">Escritório Financeiro</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card className="border-primary/30 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent">
              ...
              <CardContent className="space-y-3">
                <div className="text-2xl sm:text-3xl font-bold text-primary">
                  {formatCurrency(financials?.availableBalance || 1378000)}
                </div>
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                  ...
                  Faturamento (Out/2025)
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                <div className="text-2xl sm:text-3xl font-bold">
                  {formatCurrency(financials?.monthlyRevenue || 3249000)}
                </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <div className="text-2xl sm:text-3xl font-bold">
                  {formatCurrency(financials?.networkEarnings || 2549000)}
                </div>
                <p className="text-xs text-muted-foreground">{networkStats.totalCreators} criadoras ativas</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <div className="text-2xl sm:text-3xl font-bold">
                  {formatCurrency(financials?.directEarnings || 700000)}
                </div>
                <p className="text-xs text-muted-foreground">Suas assinaturas exclusivas</p>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
            <CardHeader>
              ...
              <CardDescription>Compartilhe seu código e construa sua rede de criadoras em 4 níveis</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">Seu Código Único</label>
                  <div className="flex gap-2">
                    <Input value={referralCode} readOnly className="flex-1 bg-muted/50 font-mono text-lg font-bold" />
                    <Button
              ...
              <div className="p-4 rounded-lg bg-background border">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="text-center p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Nível 1</p>
                  </div>
                  <div className="text-center p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
                    <p className="text-2xl font-bold text-blue-600">5%</p>
                    <p className="text-xs text-muted-foreground mt-1">Nível 2</p>
                  </div>
                  <div className="text-center p-3 rounded-lg bg-purple-500/10 border border-purple-500/20">
                    <p className="text-2xl font-bold text-purple-600">3%</p>
                    <p className="text-xs text-muted-foreground mt-1">Nível 3</p>
                  </div>
                  <div className="text-center p-3 rounded-lg bg-orange-500/10 border border-orange-500/20">
                    <p className="text-2xl font-bold text-orange-600">1%</p>
                    <p className="text-xs text-muted-foreground mt-1">Nível 4</p>
                  </div>
                </div>
          ...
          <Tabs defaultValue="overview" className="w-full">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium">Nível 1 (10%)</p>
                            <p className="text-xs text-muted-foreground">Indicações diretas</p>
                          </div>
                        </div>
                        <p className="text-2xl font-bold text-emerald-600">{networkStats.level1}</p>
                      </div>
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium">Nível 2 (5%)</p>
                            <p className="text-xs text-muted-foreground">Indicações de 2º nível</p>
                          </div>
                        </div>
                        <p className="text-2xl font-bold text-blue-600">{networkStats.level2}</p>
                      </div>
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg bg-purple-500/10 border border-purple-500/20">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium">Nível 3 (3%)</p>
                            <p className="text-xs text-muted-foreground">Indicações de 3º nível</p>
                          </div>
                        </div>
                        <p className="text-2xl font-bold text-purple-600">{networkStats.level3}</p>
                      </div>
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg bg-orange-500/10 border border-orange-500/20">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium">Nível 4 (1%)</p>
                            <p className="text-xs text-muted-foreground">Indicações de 4º nível</p>
                          </div>
                        </div>
                        <p className="text-2xl font-bold text-orange-600">{networkStats.level4}</p>
                      </div>
                    </div>

                    <div className="pt-4 border-t">
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-medium">Total de Criadoras</p>
                        <p className="text-2xl font-bold">{networkStats.totalCreators}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
            ...
            <TabsContent value="network" className="space-y-4 mt-6">
              <Card>
                <CardHeader>
                  <CardTitle>Criadoras na Minha Rede MMN</CardTitle>
                  <CardDescription>
                ...
                <CardContent>
                  <div className="overflow-x-auto">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead className="min-w-[180px]">Criadora</TableHead>
                      ...
                      <TableBody>
                        {network.length === 0 ? (
                          ...
                          network.map((creator) => {
                            const commissionRates = [0.1, 0.05, 0.03, 0.01]
            ...
            <TabsContent value="tree" className="space-y-4 mt-6">
              <Card>
                <CardHeader>
                  <CardTitle>Árvore Hierárquica da Rede MMN</CardTitle>
                  <CardDescription>
                ...
                <CardContent>
                  {networkTree.length === 0 ? (
                    <div className="text-center py-16">
                      <Network className="h-20 w-20 mx-auto mb-4 text-muted-foreground/30" />
                      <p className="text-lg font-medium text-muted-foreground mb-2">Sua rede ainda está vazia</p>
                    ...
                    <ScrollArea className="h-[600px] pr-4">
                      <div className="space-y-3">
                        {/* Root Node (You) */}
                        <div className="flex items-center gap-3 p-4 rounded-lg bg-gradient-to-r from-primary/20 to-primary/10 border-2 border-primary shadow-lg">
                          ...
                          <div className="flex-1">
                            <p className="font-bold text-lg">Você</p>
                            <p className="text-sm text-muted-foreground">@{userProfile?.username}</p>
                          </div>
                          <Badge className="bg-primary text-primary-foreground shadow-sm">Topo da Rede</Badge>
                        </div>

                        {/* Network Tree */}
                        <div className="pl-4 border-l-2 border-muted">{renderNetworkTree(networkTree)}</div>
                      </div>
                    </ScrollArea>
                  )}
                </CardContent>
