...
export function PostCard({ post, onLike, onRetweet }: PostCardProps) {
    ...
    const checkUserAccess = async () => {
      if (user) {
        ...
        if (post.requiredLevel) {
          // Se é o dono do post, sempre tem acesso
          if (isOwner) {
            setHasAccess(true)
          } else {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      const newCount = result.liked ? post.likes + 1 : post.likes - 1
      onLike?.(post.id, newCount)

      // Show XP notification if gained
      if (result.xpGained && result.xpGained > 0) {
        console.log(`[v0] XP gained from like: ${result.xpGained}`)
      }
    } catch (error) {
